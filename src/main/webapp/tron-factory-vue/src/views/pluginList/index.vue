        ...
        <el-row :gutter="12">
          <el-col :span="24">
            <el-card shadow="hover">
              ...
              <div
                v-if="transcationContentShow"
                class="transcationDialogFormContent"
              >
                <el-form-item prop="transaction" class="baseFormItem mgt20">
                  <el-checkbox-group
                    v-model="pluginTransactionForm.transaction"
                  >
                    <el-checkbox
                      :label="item.value"
                      :key="ind"
      ...
      this.$_api.pluginApi
        ...
        .then(res => {
          ...
          if (res.customTransaction != '') {
            ...
            this.pluginTransactionForm.transaction = [...res.transaction, 9]
          } else {
            this.pluginTransactionForm.transaction = res.transaction
          }
          if (res.dbEngine != 'leveldb' && res.dbEngine != 'rocksdb') {
            this.plugindbForm = {
              dbsetting: 3,
          ...
          if (this.pluginOnsensusForm.consensus == 3) {
            if (this.pluginOnsensusForm.consensusContent == '') {
              this.$message.success(
                this.$t('tronPluginCustomConsensusModulePlaceholder'),
              )
              return
            }
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          if (this.pluginTransactionForm.transactionContent != '') {
            this.pluginTransactionForm.transaction.forEach(item => {
              if (item === 9) {
                if (
                  this.pluginTransactionForm.transactionContent.length > 4 &&
                  this.pluginTransactionForm.transactionContent.slice(-4) ==
                    '.jar'
                ) {
                  checkStatus = 1
                } else {
            ...
            if (this.pluginTransactionForm.transaction.indexOf(9) > 0) {
              checkStatus = 4
              this.$message.warning(
                this.$t('tronPluginCustomTradingModulePlaceholder'),
              )
            }
          }
          console.log(checkStatus)
          if (checkStatus == 1) {
            this.pluginTransactionForm.transaction.forEach((item, ind) => {
              if (item === 9) {
                this.pluginTransactionForm.transaction.splice(ind, 1)
              } else {
                checkStatus = 3
              }
            })
          ...
          this.$_api.pluginApi
            .transactionApi({
              ...
              customTransaction: this.pluginTransactionForm.transactionContent,
            })
            .then(() => {
